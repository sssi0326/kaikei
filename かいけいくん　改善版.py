...
#--------------------------------------------------------
    if event=="kaikei":
        ...
        milk_per=int(ruikei_milk/ruikei_cnt*100)
        ichi_per=int(ruikei_ichi/ruikei_cnt*100)
        coff_per=int(ruikei_coff/ruikei_cnt*100)
        karu_per=int(ruikei_karu/ruikei_cnt*100)
        ...
        window["maxsyouhin"].update(syouhin[cnt_list.index(max(cnt_list))])
        window["maxcnt"].update(max(cnt_list))
        window["maxper"].update(max(per_list))
        window["minsyouhin"].update(syouhin[cnt_list.index(min(cnt_list))])
        window["mincnt"].update(min(cnt_list))
        window["minper"].update(min(per_list))
        for i in range(4):
            window["graph_"+flav[i]].update("|"*per_list[i])
            cnt[i]=0
            window[flav[i]+"_cnt"].update(0)
